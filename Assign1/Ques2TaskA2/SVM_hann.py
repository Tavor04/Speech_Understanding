...
def extract_spectral_features(audio_path):
    try:
        ...
        hop_length = 256
        win_length = 1024
        window = torch.hann_window(win_length)  # Hann window function
        ...
        power_spectrogram = spectrogram ** 2  # Power spectrogram for flatness computation

        # **1. Compute Spectral Centroid**
        spectral_centroid = F.spectral_centroid(waveform, sample_rate=sr, pad=0, window=window, n_fft=n_fft, hop_length=hop_length, win_length=win_length).mean()

        # **2. Compute Spectral Bandwidth (Manual Calculation)**
        freqs = torch.linspace(0, sr // 2, steps=(n_fft // 2) + 1)  # Frequency bins
        spectral_bandwidth = torch.sqrt(torch.sum(((freqs - spectral_centroid) ** 2) * spectrogram.mean(dim=-1))).mean()

        # **3. Compute Spectral Flatness (Manual Calculation)**
        geometric_mean = torch.exp(torch.mean(torch.log(power_spectrogram + 1e-6), dim=-1))  # Avoid log(0)
        arithmetic_mean = torch.mean(power_spectrogram, dim=-1)
        spectral_flatness = (geometric_mean / arithmetic_mean).mean()

        # **4. Compute Zero-Crossing Rate from waveform (not spectrogram)**
