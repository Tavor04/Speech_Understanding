...
def extract_spectral_features_hamming(audio_path):
    try:
        # Load audio file
        ...
        hop_length = 256
        win_length = 1024
        window = torch.hamming_window(win_length)  # Hamming window
...
for _, row in df.iterrows():
    ...
    if os.path.exists(file_path):  # Ensure the file exists
        features = extract_spectral_features_hamming(file_path)
